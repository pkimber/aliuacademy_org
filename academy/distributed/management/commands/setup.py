...
def raw_input_yn(prompt):
    ans = ""
    while True:
        ans = input("%s (yes or no) " % prompt.strip()).lower()
        if ans in ["yes", "no"]:
            break
...
def get_username(username):
    while not validate_username(username):
        ...
        username = input("Username (leave blank to use '%s'): " % get_clean_default_username()) or get_clean_default_username()
...
def get_hostname_and_description(hostname=None, description=None):
    ...
    while not hostname:
        ...
        hostname = input(prompt) or default_hostname
        if not hostname:
            sys.stderr.write("\tError: hostname must not be empty.\n")
        else:
            break

    description = description or input("Please enter a one-line description for this server (or, press Enter to leave blank): ")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # PJK 11/03/2015 I don't know if we need the following, but comment out for now
        ...
        if not settings.CENTRAL_SERVER:
            ...
            # PJK 11/03/2015.  I don't think we will need this...
